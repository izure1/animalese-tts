...
export class JapaneseAnalyzer implements TextAnalyzer {
  public analyze(text: string): PhonemeToken[][] {
    ...
    while (i < text.length) {
      ...
      if (KANA_MAP[combine]) {
        romajiSequence = KANA_MAP[combine]
        if (sokuon) {
          romajiSequence = romajiSequence[0] + romajiSequence
          sokuon = false
        }
        const group: PhonemeToken[] = []
        for (let j = 0; j < romajiSequence.length; j++) {
          group.push({ phoneme: romajiSequence[j], mergeWithNext: j < romajiSequence.length - 1 })
        }
        phonemes.push(group)

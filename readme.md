...
- `NodePlayer`: Saves (Exports) the synthesized audio in chunk forms to `.wav` files on the local disk in a Node.js environment.
- `BrowserPlayer`: Plays the buffer immediately in a web browser using the Web Audio API (`AudioContext`).
